...
function notificationItems() {
  ...
  if (canSee(["accounts", "sales"])) {
    ...
    state.customers.filter((item) => !item.saleId && !isPartsCustomerEntry(item) && Number(item.pending || 0) > 0).forEach((item) => items.push({ type: "Payment Pending", text: `${item.name || "-"} - ${item.mobile || item.invoice || item.bikeNo || "-"} - ${money(item.pending)}${dated(item)}`, action: "payment-customer", id: item.id }));
  }
  return items;
}
...
function bindSales() {
  ...
  qs("#bikeForm").addEventListener("submit", async (event) => {
    event.preventDefault();
...
function renderPartsStats() {
  ...
  const partsPending = state.partSales.reduce((sum, sale) => sum + Number(sale.pending || 0), 0);
  ...
  if (qs("#partsPendingTotal")) qs("#partsPendingTotal").textContent = money(partsPending);
...
function showIssuedTodayDetails() {
  ...
  box.innerHTML = rows.length
    ? `<table><thead><tr><th>Invoice / VN</th><th>Registration</th><th>Part No.</th><th>Part Name</th><th>Common Name</th><th>Issued Qty</th><th>Returned Qty</th><th>Net Qty</th><th>Amount</th><th>Paid</th><th>Pending</th><th>Action</th></tr></thead><tbody>${rows.map((sale) => {
        ...
        return `<tr><td>${sale.invoice || "-"}</td><td>${sale.registrationNo || "-"}</td><td>${part?.partNo || sale.partNo || "-"}</td><td>${sale.name || part?.name || "-"}</td><td>${sale.commonName || part?.commonName || "-"}</td><td>${issued}</td><td>${returned}</td><td>${Number(sale.qty || 0)}</td><td>${money(sale.amount)}</td><td>${money(sale.paid || 0)}</td><td class="${Number(sale.pending || 0) > 0 ? "due" : "ok"}">${money(sale.pending ?? sale.amount)}</td><td><button class="soft-btn" data-receive-parts="${reference}">Receive</button></td></tr>`;
      }).join("")}</tbody></table>`
...
function invoicePartRows(reference) {
  ...
  state.partSales.filter((sale) => sameSaleReference(sale, reference)).forEach((sale) => {
    ...
    old.paid += Number(sale.paid || 0);
    old.discount += Number(sale.discount || 0);
    old.amount = Math.max(old.qty, 0) * mrp;
    old.pending = Math.max(old.amount - old.paid - old.discount, 0);
...
function renderInvoiceParts() {
  ...
  const partsHtml = rows.length
    ? `<div class="table-wrap"><table><thead><tr><th>Date</th><th>Invoice / VN</th><th>Registration</th><th>Part No.</th><th>Part</th><th>MRP</th><th>Sold</th><th>Returned</th><th>Remaining</th><th>Amount</th><th>Paid</th><th>Pending</th></tr></thead><tbody>${rows.map((sale) => {
        const part = state.parts.find((item) => item.id === sale.partId);
        return `<tr><td>${sale.date || "-"}</td><td>${sale.invoice || "-"}</td><td>${sale.registrationNo || "-"}</td><td>${salePartNo(sale) || "-"}</td><td>${sale.name || "-"}</td><td>${money(sale.mrp || part?.sale || 0)}</td><td>${sale.originalQty || sale.qty || 0}</td><td>${sale.returnedQty || 0}</td><td>${Math.max(Number(sale.qty || 0), 0)}</td><td>${money(sale.amount)}</td><td>${money(sale.paid || 0)}</td><td class="${Number(sale.pending || 0) > 0 ? "due" : "ok"}">${money(sale.pending || 0)}</td></tr>`;
      }).join("")}</tbody></table></div>`
    : "";
  const pending = rows.reduce((sum, sale) => sum + Number(sale.pending || 0), 0);
  const paid = rows.reduce((sum, sale) => sum + Number(sale.paid || 0), 0);
  const paymentHtml = rows.length ? `<div class="notice"><strong>Parts Payment</strong>Paid: ${money(paid)} | Pending: ${money(pending)}</div>` : "";
...
function saveJobParts() {
  ...
  for (const row of rows) {
    ...
    if (existingSale) {
      ...
      existingSale.pending = Math.max(Number(existingSale.amount || 0) - Number(existingSale.paid || 0) - Number(existingSale.discount || 0), 0);
      ...
      state.partSales.push({
        ...
        paid: 0,
        discount: 0,
        pending: Math.max(qty - returnedQty, 0) * Number(part.sale || 0),
        date: today(),
        mode: "sell",
        returnedQty,
      });
...
function saveReturnParts() {
  ...
  for (const row of rows) {
    ...
    sale.pending = Math.max(Number(sale.amount || 0) - Number(sale.paid || 0) - Number(sale.discount || 0), 0);
  ...
  showToast(`${savedCount} parts returned. Parts balance ${money(returnTotal)} kam hua.`);
}
...
function sellPart(event) {
  ...
  const amount = qty * Number(part.sale || 0);
  state.partSales.push({ id: createId(), partId: part.id, name: part.name, qty, customer: textValue("#sellCustomer"), amount, paid: 0, discount: 0, pending: amount, date: today() });
  saveState();
  event.target.reset();
  renderPartsRows();
